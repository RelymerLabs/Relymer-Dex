...
export const getFarmBaseTokenPrice = (
  ...
  // console.log(farm.quoteToken.symbol, stable, wNative)
  
  if (farm.quoteToken.symbol === stable) {
    return hasTokenPriceVsQuote ? FixedNumber.from(farm.tokenPriceVsQuote) : FIXED_ONE
  }
  // console.log(hasTokenPriceVsQuote, nativePriceUSD.toString(), wNative)
...
const filterFarmsByQuoteToken = (
  farms: SerializedFarmPublicData[],
  preferredQuoteTokens: string[] = ['WETH', 'USDT'],
): SerializedFarmPublicData => {
...
export const getLpTokenPrice = (
  ...
  // console.log(lpTotalSupplyAsBigNumber.toString(), lpTotalInQuoteTokenBigNumber.toString())
...
export const getFarmsPrices = (
  ...
  : FIXED_ZERO
  // const nativePriceUSD = FixedNumber.from('1866')
  ...
  const farmsWithPrices = farms.map((farm) => {
    ...
    // console.log('busd',lpTokenPrice, tokenPriceBusd)
