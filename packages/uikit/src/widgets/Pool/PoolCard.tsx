...
export function PoolCard<T>({ pool, cardContent, aprRow, isStaked, cardFooter, tokenPairImage }: PoolCardPropsType<T>) {
  ...
  const isRoshPool = earningToken?.symbol === "ARB" && stakingToken?.symbol === "ARB";
        ...
        {totalStaked && totalStaked.gte(0) ? (
          <>
            <Pool.PoolCardHeaderTitle
              title={isRoshPool ? t("Manual") : t("Earn %asset%", { asset: earningToken?.symbol || "" })}
              subTitle={
                isRoshPool ? t("Earn BNB, stake BNB") : t("Stake %symbol%", { symbol: stakingToken?.symbol || "" })
              }
            />
            {tokenPairImage}
          </>
