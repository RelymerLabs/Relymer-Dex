...
function farmLpTransformer(farmResult, masterChefResult) {
  return (farm, index) => {
    ...
    const [info, oscarTotalAllocPoint] = masterChefResult[index]
    const allocPoint = info ? new BigNumber(info.allocPoint?._hex) : BIG_ZERO
    const poolWeight = oscarTotalAllocPoint ? allocPoint.div(new BigNumber(oscarTotalAllocPoint)) : BIG_ZERO
