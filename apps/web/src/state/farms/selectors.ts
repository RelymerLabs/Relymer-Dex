...
const deserializeFarmUserData = (farm: SerializedFarm): DeserializedFarmUserData => {
  ...
  return {
    ...
    earningsWeth: farm.userData ? new BigNumber(farm.userData.earningsWeth) : BIG_ZERO,
    earningsStableCoin: farm.userData ? new BigNumber(farm.userData.earningsStableCoin) : BIG_ZERO,
    proxy: {
...
export const makeUserFarmFromPidSelector = (pid: number) =>
  createSelector([selectFarmByKey('pid', pid)], (farm) => {
    const { allowance, tokenBalance, stakedBalance, earnings, earningsWeth, earningsStableCoin, proxy } = deserializeFarmUserData(farm)
    return {
      allowance,
      tokenBalance,
      stakedBalance,
...
export const farmSelector = (chainId: number) =>
  createSelector(
    ...
    (farms) => {
      // console.log(chainId,farms)
      const deserializedFarmsData = farms.data.map(deserializeFarm).filter((farm) => farm.token.chainId === chainId)
      const { loadArchivedFarmsData, userDataLoaded, poolLength, oscarPerSec } = farms
      // console.log(oscarPerSec, poolLength, oscarPerSec) 
