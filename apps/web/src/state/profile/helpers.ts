...
import { RoshiniProfile } from 'config/abi/types/RoshiniProfile'
...
const transformProfileResponse = (
  profileResponse: Awaited<ReturnType<RoshiniProfile['getUserProfile']>>,
): Partial<Profile> => {
...
export const getProfile = async (address: string): Promise<GetProfileResponse> => {
  try {
    const profileCalls = ['hasRegistered', 'getUserProfile'].map((method) => {
      return { address: getRoshiniProfileAddress(), name: method, params: [address] }
    })
