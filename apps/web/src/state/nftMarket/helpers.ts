...
import { RoshiniBunniesAddress } from 'views/Nft/market/constants'
...
/**
 * Fetch market data for RoshiniBunnies NFTs by bunny id using the Subgraph
...
export const getNftsByBunnyIdSg = async (
  ...
  try {
    ...
    const res = await request(
      ...
      `,
      {
        collectionAddress: RoshiniBunniesAddress.toLowerCase(),
        where,
        orderDirection,
      },
    )
...
export const getAllRoshiniBunniesLowestPrice = async (bunnyIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleRoshiniBunnySubQueries = bunnyIds.map(
      (
        bunnyId,
    ...
    const rawResponse: Record<string, { currentAskPrice: string }[]> = await request(
      GRAPH_API_NFTMARKET,
      gql`
        query getAllRoshiniBunniesLowestPrice {
          ${singleRoshiniBunnySubQueries}
        }
      `,
    )
    return fromPairs(
    ...
    console.error('Failed to fetch RoshiniBunnies lowest prices', error)
    return {}
  }
}

export const getAllRoshiniBunniesRecentUpdatedAt = async (bunnyIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleRoshiniBunnySubQueries = bunnyIds.map(
      (
        bunnyId,
    ...
    console.error('Failed to fetch RoshiniBunnies latest market updates', error)
    return {}
  }
}
...
export const getMetadataWithFallback = (apiMetadata: ApiResponseCollectionTokens['data'], bunnyId: string) => {
  ...
  return (
    apiMetadata[bunnyId] ?? {
      name: '',
      description: '',
      collection: { name: 'Roshini Bunnies' },
      image: {
        original: '',
        thumbnail: '',
      },
...
export const getRoshiniBunniesAttributesField = (bunnyId: string) => {
