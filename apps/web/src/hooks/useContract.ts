...
import {
  ...
  getNftSaleContract,
  getRoshiniBunniesContract,
  getRoshiniSquadContract,
...
import IRoshiniPairABI from 'config/abi/IRoshiniPair.json'
import multiCallAbi from 'config/abi/Multicall.json'
import WETH_ABI from 'config/abi/weth.json'
import { getContract } from 'utils'

import { IRoshiniPair } from 'config/abi/types/IRoshiniPair'
...
export const useRoshiniBunnies = () => {
  const { data: signer } = useSigner()
  return useMemo(() => getRoshiniBunniesContract(signer), [signer])
}
...
export const usePredictionsContract = (address: string, tokenSymbol: string) => {
  ...
  return useMemo(() => {
    ...
    const getPredContract = tokenSymbol === 'AETH' ? getCakePredictionsContract : getPredictionsContract
...
export const useRoshiniSquadContract = () => {
  const { data: signer } = useSigner()
  return useMemo(() => getRoshiniSquadContract(signer), [signer])
}
...
export function usePairContract(pairAddress?: string, withSignerIfPossible?: boolean): IRoshiniPair | null {
  return useContract(pairAddress, IRoshiniPairABI, withSignerIfPossible)
}
