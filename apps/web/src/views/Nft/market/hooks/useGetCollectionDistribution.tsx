...
import { getRoshiniBunniesAddress } from 'utils/addressHelpers'
...
export const useGetCollectionDistributionPB = () => {
  ...
  useEffect(() => {
    const fetchTokens = async () => {
      ...
      try {
        apiResponse = await getNftsFromCollectionApi(RoshiniBunniesAddress)
        if (!apiResponse) {
      ...
      const bunnyCountCalls = tokenIds.map((tokenId) => ({
        address: getRoshiniBunniesAddress(),
        name: 'bunnyCount',
        params: [tokenId],
      }))
      try {
        const response = await multicallv2({ abi: RoshiniBunniesAbi, calls: bunnyCountCalls })
