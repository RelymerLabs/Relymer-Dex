...
import { getRoshiniProfileAddress } from 'utils/addressHelpers'
...
const useNftOwner = (nft: NftToken, isOwnNft = false) => {
  ...
  const roshiniProfileAddress = getRoshiniProfileAddress()
  ...
  useEffect(() => {
    const getOwner = async () => {
      try {
        if (isOwnNft && account) {
          setOwner(account)
        } else if (tokenOwner && isAddress(tokenOwner) !== isAddress(roshiniProfileAddress)) {
          setOwner(tokenOwner)
        } else {
          setOwner(null)
        }
  ...
  }, [account, isOwnNft, currentSeller, collectionContract, tokenId, tokenOwner, roshiniProfileAddress])
