...
import { getFarmMonkRewardApr, SerializedFarmConfig } from '@pancakeswap/farms'
  ...
  try {
    ...
    const finalFarm = farmsWithPrice.map((f) => {
      return {
        ...
        monkApr: getFarmMonkRewardApr(f, FixedNumber.from(cakeBusdPrice.toSignificant(3)), oscarPerSec),
      }
    }) as FarmResult
